...
namespace ConsoleFight
{
    ...
    internal class Fight
    {
        ...
        internal static void WriteInfo(int distance, int playerActRange, int enemyAtcRange, PlayerClass player)
        {
            ...
            player.Inventory.ShowPotions(true);
    ...
    internal class FightWithoutAbilities
    {
        internal static int StartFight(ref PlayerClass player)
        {
            ...
            int startDamage = player.Weapon.Damage;
            ...
            while (baseEnemy.HP > 0 && player.HP > 0)
            {
                ...
                {
                    startDamage = player.Weapon.Damage;
                    dealtDamage = PlayerTurn(ref player, ref distance, ref rnd, ref enemyEvadeChance, ref logQueue, ref baseEnemy, ref enemySkipDueAtcRange, ref dealtDamage, startDamage);
                ...
                if (baseEnemy.HP > 0)
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            {
                                enemyDealtDamage = (baseEnemy.Damage);
                                player.GetDamage(enemyDealtDamage);
                                WriteLogs(logQueue, $"{baseEnemy.Name} атакует. {player.Name} получает {enemyDealtDamage} урона.");
                            }
                        }
        ...
        internal static void WriteActions(PlayerClass player, ConsoleColor actionsColor, int dealtDamage)
        {
            int startX = 38;
            int startY = 13;
            Console.CursorVisible = false;
            ...
            Console.SetCursorPosition(startX, startY + 1);
            Console.ForegroundColor = ConsoleColor.DarkYellow;
            Console.Write($"2. Применить {player.ActiveAbility.Name} (недоступно)");
        ...
        internal static void WriteActions(PlayerClass player, ConsoleColor actionsColor)
        {
            var clear = new string(' ', 50);
            int startX = 38;
            int startY = 13;
        ...
        internal static ConsoleKey GetPlayerAction(PlayerClass player)
        {
            List<ConsoleKey> actions = new List<ConsoleKey> { ConsoleKey.D1 };
        ...
        public static int PlayerTurn(ref PlayerClass player, ref int distance, ref Random rnd, ref int enemyEvadeChance, ref Queue<string> logQueue, ref BaseEnemy baseEnemy, ref int enemySkipDueAtcRange, ref int dealtDamage, int startDamage)
        {

            WriteActions(player, ConsoleColor.Yellow, startDamage);
            ConsoleKey playerAction = GetPlayerAction(player);
            switch (playerAction)
            {
                case ConsoleKey.D1:
                    int playerChanceToMiss = rnd.Next(0, 101);
                ...
                case ConsoleKey.C:
                    ...
                    WriteActions(player, ConsoleColor.Yellow, startDamage);
                    WriteLogs(logQueue);
                    PlayerTurn(ref player, ref distance, ref rnd, ref enemyEvadeChance, ref logQueue, ref baseEnemy, ref enemySkipDueAtcRange, ref dealtDamage, startDamage);
